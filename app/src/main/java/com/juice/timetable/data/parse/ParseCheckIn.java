...
public class ParseCheckIn {
     ...
     * 获取自己签到信息
     */
    public static MyCheckInInfo getMySigned() throws ParseException {
        //定义时间类
        ...
        MyCheckInInfo myCheckInInfo = new MyCheckInInfo();
        for (Element element : elements) {
            ...
            for (Element el : ele) {
                ...
                Date checkTime = sdf.parse(el.getElementsByTag("td").eq(1).text());
                myCheckInInfo.setCheckTime(checkTime.toString());
                if ("未签".equals(el.getElementsByTag("td").eq(2).text())) {
                    boolean isCheckIn = false;
                    myCheckInInfo.setCheckIn(isCheckIn);
                } else {
     ...
     * 获取班级签到信息
     */
    public static List<ClassNoSignedItem> getClassNoSigned() {
        //抓取
        //String s = ReadFile.readToString("C:\\Users\\14989\\Desktop\\网页内容\\未签列表.html");
        String s = getClassItemStr();
        //解析文档
        Document doc = Jsoup.parse(s);
        //提取表格
        Elements elements = doc.getElementsByTag("tbody");
        ...
        for (Element element : elements) {
            ...
            for (int a = 0; a < len_Tr; a++) {
                ...
                for (Element el : ele) {
                    //tr标签中td的数量
                    Integer sno = Integer.valueOf(el.getElementsByTag("td").eq(2).text());
                    cnsi.setSno(sno);
    ...
    public static String getClassItemStr() {
